...
//   */
//  def get(fqon: String, provider: UUID, path: String): Action[AnyContent] = AsyncAudited(fqon) { implicit request =>
...
//   */
//  def post(fqon: String, provider: UUID, path: String): Action[AnyContent] = AsyncAudited(fqon){ implicit request =>
//    val headers = request.headers.toMap
//    (for {
