...
@Singleton
class SystemConfigActor @Inject() extends Actor with ActorLogging {
  ...
  override def supervisorStrategy: SupervisorStrategy = OneForOneStrategy(
    ...
    case e: Exception =>
      log.error(e, "oh no, my child crashed")
      SupervisorStrategy.Stop
  }

  override def receive: Receive = {
    case sk: SetKeys =>
      val f = (context.actorOf(Props(new ConfigGetter)) ? sk)
        .mapTo[Boolean]
      f pipeTo sender()
    case GetKey(key) =>
      val f = (context.actorOf(Props(new ConfigGetter)) ? GetAllConfig)
...
object SystemConfigActor {
  ...
  case class SetKeys(pairs: (String,String)*)
  case class GetKey(key: String)
  case object GetAllConfig
}

class ConfigGetter extends Actor with ActorLogging {
  ...
  def updateConfig(data: Map[String,String]) = {
    ResourceFactory.findById(ResourceIds.SystemConfig).fold {
      ...
      ResourceFactory.create(ResourceIds.Org, root.id)(
      ...
      sc =>
        ...
        val data = Json.parse(props.getOrElse("data", "{}")).as[JsObject]
        val updatedData = data ++ Json.toJson(data).as[JsObject]
  ...
  override def receive: Receive = {
    case SystemConfigActor.SetKeys(pairs) =>
      updateConfig(Map(pairs))
      sender() ! true
