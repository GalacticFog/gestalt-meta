...
class KubeServiceSpec extends PlaySpecification with ResourceScope with BeforeAll with BeforeAfterEach with JsonMatchers {
  ...
  abstract class FakeKubeCreate( force_pull: Boolean = true,
    ...
    lazy val testSetup = {
      ...
      mockSkuber.list()(any,meq(skuber.Service.svcListDef)) returns Future.successful(new skuber.ServiceList("","",None,List(mockService)))
      mockSkuber.list(any)(any,meq(skuber.Service.svcListDef)) returns Future.successful(new skuber.ServiceList("","",None,List(mockService)))
      ...
      mockSkuber.create(any)(any,meq(skuber.Service.svcDef)) returns Future.successful(mockService)
      mockSkuber.create(any)(any,meq(skuber.Secret.secDef)) returns Future.successful(mockSecret)

      mockSkuber.getOption(meq(metaContainer.name))(any,meq(skuber.Service.svcDef)) returns Future.successful(Some(mockService))
  ...
  "KubernetesService" should {
    ...
    "configure environment variables for containers" in new FakeKube {
      ...
      testSetup.kubeClient.create[Deployment](any)(any,any) returns Future.successful(mock[skuber.ext.Deployment])
      ...
      there was one(testSetup.kubeClient).create[Deployment](argThat(
    ...
    "deploy service for exposed port mappings and set service addresses and host port" in new FakeKubeCreate(
      ...
      there was one(testSetup.kubeClient).create[Service](serviceCaptor.capture())(any,any)
    ...
    "create and mount persistent volume claims when mounting into container" in new FakeKubeCreate(
      ...
      testSetup.kubeClient.list()(any,meq(PersistentVolumeClaim.pvcListDef)) returns Future.successful(new skuber.PersistentVolumeClaimList("","",None,Nil))
      testSetup.kubeClient.create[PersistentVolumeClaim](any)(any,any) returns Future.successful(mock[skuber.PersistentVolumeClaim])
      ...
      there was one(testSetup.kubeClient).create[PersistentVolumeClaim](argThat(
        inNamespace(testSetup.testNS.name)
          and
    ...
    "fail to create container if persistent volume claim creation fails" in new FakeKubeCreate(
      ...
      testSetup.kubeClient.list()(any,meq(PersistentVolumeClaim.pvcListDef)) returns Future.successful(new skuber.PersistentVolumeClaimList("","",None,Nil))
      testSetup.kubeClient.create[PersistentVolumeClaim](any)(any,any) returns Future.failed(new skuber.K8SException(mock[client.Status]))
    ...
    "provision secrets and containers with the requested labels and meta-specific labels" in new FakeKubeCreate(labels = Map(
      ...
      there was one(testSetup.kubeClient).create[Secret](argThat(
    ...
    "provision namespace with the expected name and labels" in new FakeKube {
      ...
      testSetup.kubeClient.getOption(newEnv.id.toString)(any,meq(skuber.Namespace.namespaceDef)) returns Future.successful(None)
      ...
      there was one(testSetup.kubeClient).create[skuber.Namespace](argThat(
    ...
    "orchestrate kube ingress for virtual hosts" in new FakeKubeCreate(port_mappings = Seq(
      ...
      testSetup.kubeClient.create[Ingress](any)(any,any) returns Future(mock[Ingress])
      ...
      there was one(testSetup.kubeClient).create[Ingress](argThat(
        inNamespace(testSetup.testNS.name)
          and
    ...
    "delete service and any ingress on container delete" in new FakeKube {
      ...
      testSetup.kubeClient.delete(mockDep.name)(Deployment.deployDef) returns Future.successful(())
      testSetup.kubeClient.delete(mockRS.name)(ReplicaSet.rsDef) returns Future.successful(())
      testSetup.kubeClient.delete(mockService.name)(Service.svcDef) returns Future.successful(())
      testSetup.kubeClient.delete(mockIngress.name)(Ingress.ingDef) returns Future.successful(())

      await(testSetup.kubeService.destroy(metaContainer))
      there was one(testSetup.kubeClient).delete(mockDep.name)(Deployment.deployDef)
      there was one(testSetup.kubeClient).delete(mockRS.name)(ReplicaSet.rsDef)
      there was one(testSetup.kubeClient).delete(mockService.name)(Service.svcDef)
      there was one(testSetup.kubeClient).delete(mockIngress.name)(Ingress.ingDef)
      there was one(testSetup.kubeClient).close
    }
    ...
    "not fail if no service or ingress to delete on container delete" in new FakeKube {
      ...
      await(testSetup.kubeService.destroy(metaContainer))
      there were no(testSetup.kubeClient).delete(any)(Deployment.deployDef)
      there was one(testSetup.kubeClient).close
    }
    ...
    "delete empty ingress on container PUT" in new FakeKubeWithPrexistingContainer(Seq(
      ...
      testSetup.kubeClient.getOption(meq(metaContainer.name))(any,meq(Ingress.ingDef)) returns Future.successful(Some(mock[skuber.ext.Ingress]))
      testSetup.kubeClient.list(any)(any,meq(Deployment.deployListDef)) returns Future.successful(new skuber.ext.DeploymentList("","",None,List(mock[skuber.ext.Deployment])))
      testSetup.kubeClient.delete(metaContainer.name)(Ingress.ingDef) returns Future.successful(())
      testSetup.kubeClient.getOption(meq(metaContainer.name))(any,meq(Service.svcDef)) returns Future.successful(Some(mock[skuber.Service]))
      testSetup.kubeClient.update(any)(any,meq(Service.svcDef)) returns Future.successful(mock[skuber.Service])
      ...
      val Some(updatedContainerProps) = updatedContainer.properties
      there was one(testSetup.kubeClient).delete("test-container")(Ingress.ingDef)
    ...
    "delete empty service on container PUT" in new FakeKubeWithPrexistingContainer(Seq(
      ...
      testSetup.kubeClient.list(any)(any,meq(Deployment.deployListDef)) returns Future.successful(new skuber.ext.DeploymentList("","",None,List(mock[skuber.ext.Deployment])))
      testSetup.kubeClient.getOption(meq(metaContainer.name))(any,meq(Service.svcDef)) returns Future.successful(Some(mock[skuber.Service]))
      testSetup.kubeClient.delete(metaContainer.name)(Service.svcDef) returns Future.successful(())
      ...
      val Some(updatedContainerProps) = updatedContainer.properties
      there was one(testSetup.kubeClient).delete(metaContainer.name)(Service.svcDef)
      there was one(testSetup.kubeClient).update(any)(any,meq(Deployment.deployDef))
      there were two(testSetup.kubeClient).close
    }
    ...
    "create service and ingress on container update" in new FakeKubeWithPrexistingContainer(Seq(
      ...
      testSetup.kubeClient.getOption(meq(metaContainer.name))(any,meq(Ingress.ingDef)) returns Future.successful(None)
      testSetup.kubeClient.list(any)(any,meq(Deployment.deployListDef)) returns Future.successful(new skuber.ext.DeploymentList("","",None,List(mock[skuber.ext.Deployment])))
      testSetup.kubeClient.create[Ingress](any)(any,any) returns Future.successful(mock[skuber.ext.Ingress])
      testSetup.kubeClient.getOption(any)(any,meq(Service.svcDef)) returns Future.successful(None)
      testSetup.kubeClient.create[Service](any)(any,any) returns Future.successful(mock[skuber.Service])
      ...
      there was one(testSetup.kubeClient).create[Service](argThat(
        inNamespace(testSetup.testNS.name)
          and haveName(metaContainer.name)
    ...
    "delete secret" in new FakeKube {
      ...
      testSetup.kubeClient.delete(mockSecret.name)(skuber.Secret.secDef) returns Future.successful(())

      await(testSetup.kubeService.destroySecret(metaSecret))
      there was one(testSetup.kubeClient).delete(mockSecret.name)(skuber.Secret.secDef)
      there was one(testSetup.kubeClient).close
    }
    ...
    "mount specified secrets on container create" in new FakeKubeCreate(
      secrets = Seq(
        ...
        SecretFileMount(null, "/dir/files/part-a", "part-a"),
        SecretFileMount(null, "/dir/files/part-b", "part-b"),
      ...
      there was one(testSetup.kubeClient).create[Deployment](deploymentCaptor.capture())(any,any)
      ...
      createdDeployment.spec.get.template.get.spec.get.containers.head.volumeMounts must containAllOf(Seq(
        ...
        skuber.Volume.Mount(partsVolName, "/dir", true)
      ))
